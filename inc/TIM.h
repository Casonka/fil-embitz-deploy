...
#include "FilConfig.h"
    ...
    */
    ...
    /*!
    *   @brief TimOnePulseConfigure(TIM,ch1,ch2,POLAR1,ch3,ch4,POLAR2) - configuration timer in One-Pulse Mode
    *       @arg TIM - number of timer
    *       @arg ch<n> - activate/deactivate the channels of timer
    *       @arg polarity<n> - set polarity level
    */
...
//-----------------------Simple commands reset and set the state-------------------------------------------------------------------------------------------------------------//
    ...
    #define ConfTimARR(TIM,AUTORESET)                               { TIM->ARR = ((uint32_t)(TIM->ARR | ((uint32_t)(0xFFFFFFFF))) & ((uint32_t)AUTORESET));}
...
//----------------------------------------Get state----------------------------------------------------//
    ...
    #define GetTimARR(TIM) ((uint16_t)(TIM->ARR))
    #define GetTimSR(TIM)  ((uint32_t)(TIM->SR))
    #define GetTimPSC(TIM) ((uint16_t)(TIM->PSC))
//----------------------------------------Set state----------------------------------------------------//
...
/*!
*   @brief delay_ms(uint32_t ticks) - function for delay in milliseconds
*       @return nothing
*/
void delay_ms(uint32_t ticks);

/*!
*   @brief delay_sec(uint32_t ticks) - function for delay in seconds
*       @return nothing
*/
